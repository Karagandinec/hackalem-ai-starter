...
Заготовка под хакатон **HackAlem AI** (Астана, МВЦ «EXPO»).
...
**Даты** (с [hackalem.ai](https://hackalem.ai/)): 23.09 — офлайн-этап; 24–28.09 — жюри и
технические эксперты проверяют присланные проекты; 29.09 — Demo Day; 01.10 — награждение
на Digital Bridge. Код смотрят не только на сцене, поэтому репозиторий должен запускаться
у постороннего человека с первого раза — это ровно то, что проверяет `make check`.
...
- **разметка записей моделью** с записью результата обратно в базу (`ai_label`, `ai_score`)
...
- страница данных: импорт CSV, разметка моделью в один клик, выгрузка результата
...
│       └── test_data_flow.py      # импорт CSV -> разметка моделью -> выгрузка
└── frontend/src/
...
4. **Настроить агента.** Промпт — `SYSTEM_PROMPT` в `backend/app/ai/agent.py`, число раундов
   инструментов — `MAX_TOOL_ROUNDS` там же. Нужен свой инструмент — добавить в
   `backend/app/ai/tools.py` (описание + функция + `TOOL_HANDLERS`).
5. **Подстроить разметку.** Что именно модель проставляет в `ai_label` / `ai_score` — задаётся
...
6. **Схема ответа модели.** Если нужен строгий JSON — добавить схему в
...
| POST  | `/api/entities/import`  | загрузка CSV (multipart, поле `file`)            |
| GET   | `/api/entities/export.csv` | выгрузка отфильтрованного списка в CSV        |
| GET   | `/api/dashboard?days=30`| 4 карточки + ряд для графика                     |
| POST  | `/api/ai/chat`          | чат с агентом, поток SSE                         |
| POST  | `/api/ai/enrich`        | разметить записи моделью, результат — в базу     |
